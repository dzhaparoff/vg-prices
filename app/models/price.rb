...
class Sheet
  ...
  def parse_offers!
    ...
    self.first_row.upto(self.last_row) do |row|
      ...
      sku = sheet.try(:formatted_value, row, self.price_config.sku_column).nil? ? sheet.cell(row, self.price_config.sku_column) : sheet.formatted_value(row, self.price_config.sku_column)
